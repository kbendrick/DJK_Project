...
func _rotate_columns_one_position() -> void:
	for column_index in GRID_SIZE:
		if column_index % 2 == 0:
			...
			column_offsets[column_index] = wrapi(column_offsets[column_index] - 1, 0, LETTERS_PER_COLUMN)
		else:
...
func _update_grid() -> void:
	...
	for row_index in GRID_SIZE:
		for column_index in GRID_SIZE:
			...
			var letter_index := wrapi(row_index + column_offsets[column_index],0,LETTERS_PER_COLUMN)
